...
def run_extended_benchmark():
    ...
    for dataset_name, results in all_results.items():
        ...
        if nosn['diverged']:
            nosn_diverged += 1
            sn_wins += 1
            print(f"{dataset_name:<15} {sn['test_acc']:<12.1f} {'DIVERGED':<12} {'-':<10} {sn['final_lipschitz']:<8.2f} {'∞':<8}")
        elif sn['diverged']:
            print(f"{dataset_name:<15} {'DIVERGED':<12} {nosn['test_acc']:<12.1f} {'-':<10} {'∞':<8} {nosn['final_lipschitz']:<8.2f}")
        else:
            diff = sn['test_acc'] - nosn['test_acc']
            total_diff += diff
            comparisons += 1
