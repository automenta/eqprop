...
def benchmark_gpu(input_dim, hidden_dim, output_dim, n_samples, epochs, max_steps):
    ...
    torch_model = LoopedMLP(input_dim, hidden_dim, output_dim, 
                            use_spectral_norm=True, max_steps=max_steps).to(device)
    ...
    print("  CuPy Kernel (GPU) training...", end="", flush=True)
    kernel = CuPyKernel(input_dim, hidden_dim, output_dim, 
                        max_steps=max_steps, lr=0.001,
                        use_spectral_norm=True, use_gpu=True)
    ...
    kernel_train_acc = kernel.evaluate(X_train_np, y_train_np)
    kernel_test_acc = kernel.evaluate(X_test_np, y_test_np)
    
    results['kernel'] = {
        'train_acc': kernel_train_acc,
        'test_acc': kernel_test_acc,
        'time': kernel_time,
    ...
    # Compare
    results['comparison'] = {
        'acc_gap': abs(pt_test_acc - kernel_test_acc) * 100,
...
def main():
    ...
    for scenario in scenarios:
        ...
        try:
            ...
            print(f"  {'CuPy Kernel':12} | Acc: {k['train_acc']*100:.1f}%/{k['test_acc']*100:.1f}% | "
    ...
    if avg_acc_gap < 5 and avg_speedup > 1.0:
        print("\n✅ RECOMMENDATION: Use CuPy kernel - faster AND equivalent accuracy")
    elif avg_acc_gap < 5 and avg_speedup > 0.8:
        print("\n⚠️  RECOMMENDATION: CuPy kernel competitive, use for memory benefits")
    else:
        print("\n❌ RECOMMENDATION: Keep PyTorch for now")
